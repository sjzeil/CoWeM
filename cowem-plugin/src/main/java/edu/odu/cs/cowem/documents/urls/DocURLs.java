...
public class DocURLs implements SpecialURL {
	 ...
	 */
	private static Logger logger = LoggerFactory.getLogger(DocURLs.class);
	...
	/*
	 * Maps imported document names to URLs
	 */
	private Map<String, String> importedDocCache;
			...
			if (documentName.contains("/")) {
				...
				if (siteURL != null) {
					...
					if (siteURL.startsWith("https://") || siteURL.startsWith("http://")) {
						// imported site must provide a redirection service
						...
						// imported site info is a file mapping document names to URLs
						fillImportDocCache(siteName, siteURL);
						String replacementURL = importedDocCache.get(siteName + '\t' + documentName);
						if (replacementURL != null) {
							link.setAttribute(linkAttr, replacementURL);
						} else {
							logger.warn("Could not import URL shorthand: @" + linkAttr + "=" + url);
						}
					}
				} else {
					logger.warn(
	...
	private void fillImportDocCache(String siteName, String siteURL) {
		if (importedDocCache.get(siteName + "\t\t" ) == null) {
			File importInfo = new File(siteURL);
			if (importInfo.exists()) {
				try (BufferedReader in = new BufferedReader(new FileReader(importInfo))) {
					String line = in.readLine();
					while (line != null) {
						int commaPos = line.indexOf(',');
						if (commaPos >= 0) {
							String docName = line.substring(0, commaPos);
							siteURL = line.substring(commaPos + 1, line.length());
							importedDocCache.put(siteName + '\t' + docName, siteURL);
						}
						line = in.readLine();
					}
					importedDocCache.put(siteName + "\t\t", "!");
				} catch (IOException e) {
					logger.warn("Could not read from " + importInfo + ": " + e.getMessage());
				}
