...
<!-- sectionNumbering -->
    ...
    <xsl:copy>
      ...
      <xsl:if test="(local-name(..) = 'body') and (number($numberDepth) &gt; 0)">
	    <xsl:variable name="sectionNumber">
	      <xsl:value-of select="number($FirstSection) + count(preceding-sibling::h1[text() != 'Preamble'])"/>
	    </xsl:variable>
  ...
  <xsl:template match="h2" mode="sectionNumbering">
	...
	<xsl:variable name="sectionNumber">
	  <xsl:value-of select="number($FirstSection) - 1 + count(preceding-sibling::h1[text() != 'Preamble'])"/>
	</xsl:variable>
