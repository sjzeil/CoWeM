...
-->
  ...
  <xsl:template match="item" mode="resources">
	...
	<xsl:when test="@targetdoc">
	  <xsl:value-of select="concat($webContentDir, '/Public/', 
	...
	<BODY>
	  <TEXT/>
	  <xsl:choose>
	    <xsl:when test="starts-with($linkHref,$webContentDir)">
	      <TYPE value="S"/>
	    </xsl:when>
	    <xsl:otherwise>
	      <TYPE value="H"/>
	...
	<VERSION value="3"/>
        <EXTENDEDDATA/>
	<FILES>
	  <xsl:if test="starts-with($linkHref,$webContentDir)">
	    <FILE id="{$fileID}">
	      <NAME>
  ...
  <xsl:template name="fileID">
    ...
    <xsl:value-of select="concat('_',$decID,'_2')"/>
  </xsl:template>
