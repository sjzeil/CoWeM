...
>
  ...
  <xsl:template match="section">
  	  <xsl:variable name="header" select="sectionHeader"/>
      ...
      <xsl:variable name="descriptiveContent">
          <xsl:choose>
              <xsl:when test="count($activityLists) = 0">
                  <xsl:value-of select="sectionContent/node()[local-name() != 'section']"/>
              </xsl:when>
              <xsl:otherwise>
                  <xsl:value-of select="$activityLists[1]/preceding-sibling::node()[local-name() != 'section']"/>
              </xsl:otherwise>
          </xsl:choose>
      </xsl:variable>
      <xsl:variable name="activityContent">
          <xsl:if test="count($activityLists) != 0">
               <xsl:value-of select="$activityLists[1] | $activityLists[1]/following-sibling::node()[local-name() != 'section']"/>
          </xsl:if>
      </xsl:variable>
